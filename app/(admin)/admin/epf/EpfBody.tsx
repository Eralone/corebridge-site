 ...
 *
 * Список версий сейчас недоступен: `GET /admin/epf/versions` отвечает 500, потому
 * что выборка обращается к колонкам `is_public` и `tenant_id`, которых в таблице
 * нет (промт S13 §3). Публикация и откат при этом работают — они принимают
 * конфигурацию и версию явно. Поэтому экран не пустой: он даёт сделать действие
 * и прямо говорит, что список не пришёл.
 */
