...
BOT_SAME_NET_VISITORS = 4  # адресов одной сети /16 с одним UA за день
...
def mark_bots(conn) -> int:
    ...
    # Проверяльщик ссылок площадки. Обнаружен 10.09: после каждой публикации
    # в Дзене по нашей размеченной ссылке приходил десяток «посетителей» -
    # у каждого ровно один запрос, все с разных адресов одной сети Яндекса,
    # один и тот же UA, без реферера. Дзен показал за те же сутки 26 показов
    # и 5 открытий, то есть шесть «переходов» физически не могли быть людьми.
    #
    # Признак общий и не привязан к Яндексу: несколько одноразовых посетителей
    # из одной сети /16 с одинаковым UA в один день. Живые читатели приходят
    # из разных сетей и открывают больше одной страницы.
    # Считаем в Python, а не одним UPDATE: конструкция вида
    # `(выражение, выражение) IN (SELECT ...)` в SQLite молча не срабатывает
    # на json_extract, и правило не ловило ничего - проверено 10.09.
    groups: dict[tuple, list[str]] = {}
    for row in conn.execute(
        "SELECT visitor_id, day, json_extract(meta,'$.net') net, "
        "json_extract(meta,'$.ua') ua FROM events "
        "WHERE origin = 'nginx' AND json_extract(meta,'$.net') IS NOT NULL"
    ):
        groups.setdefault((row["net"], row["ua"], row["day"]), []).append(row["visitor_id"])

    checkers: set[str] = set()
    for visitors in groups.values():
        uniq = set(visitors)
        # Четыре и больше разных адреса одной сети /16 с одним и тем же UA
        # за сутки. Условия «по одному запросу на адрес» тут нет намеренно:
        # проверялось 10.09 на живых данных, семь адресов Дзена сделали
        # 31 запрос, и ограничение на число запросов их пропускало.
        #
        # Ложное срабатывание возможно у мобильного оператора, который меняет
        # адрес внутри своей сети. При наших трёх визитах в сутки четыре
        # разных адреса одной сети с одинаковым UA за день - это почти
        # наверняка парк машин, а не четыре читателя у одного оператора.
        if len(uniq) >= BOT_SAME_NET_VISITORS:
            checkers |= uniq
    if checkers:
        conn.executemany("UPDATE events SET is_bot = 1 WHERE visitor_id = ? AND is_bot = 0",
                         [(v,) for v in checkers])
