...
| Промты бэкенду S1–S9 | ✅ S1–S7 реализованы, S8/S9 ждут |
| Спеки P01–P11 и механики MC1–MC14 | 🔄 скачан P01 |
| Каркас Next.js пересобран под E0 | ⬜ |
| Next.js слушает 3005 (сейчас `corebridge.ru` отдаёт заглушку 502) | ⬜ |
| vhost `admin.corebridge.ru` | ⬜ |

### Открытые зависимости

1. **Robokassa ждёт юридические страницы.** Дмитрий: merchant ID не выдают, пока на сайте нет
   документов. Значит `oferta`/`privacy`/`terms` + лендинг надо выкатить **до** биллинга →
   этап Э8a поднят наверх, см. план ниже.
2. **У домена нет MX** — `info@corebridge.ru` письма не принимает. У Энтерпрайза в дизайне стоит
   `mailto:info@corebridge.ru`, такое письмо не дойдёт. **Решение: заменяю `mailto` на форму
   `POST /lk/contact` с `source: "pricing"`** (рекомендация сервера) — заявка не потеряется и
   получит номер `REQ-NNNN`.
...
> **Порядок пересмотрен 2026-07-27.** Э8 (юр. страницы и лендинг) поднят на второе место:
> без них Robokassa не выдаёт merchant ID, а без него не работает биллинг. Auth сдвинут на третье
> место — он полностью готов на сервере и не блокирует ничего, кроме себя.

### Э0 — Каркас и запуск сервера ← ТЕКУЩИЙ
...
### Э8a — Юридические страницы + лендинг ← РАЗБЛОКИРУЕТ ROBOKASSA

`oferta`, `privacy`, `terms` (на `legal.css`), `index` (лендинг), `contacts`, `for-business`.
Всё ⚪ SSG, кроме форм заявок → `POST /lk/contact` ✅ real (`202 { received, ref: "REQ-NNNN" }`,
honeypot, rate-limit 5/час). Метатеги, `sitemap.xml`, `robots.txt`.

Как только страницы на проде — Дмитрий регистрируется в Robokassa, получает merchant ID,
прописывает `ROBOKASSA_*` в `.env` → разблокируется Э4.
...
### Э4 — Billing + pricing + лендинг

`GET /lk/billing`, `POST /lk/billing/pay` ✅ real (обязательна заглушка на `payment_url: null` —
Robokassa не настроена). Прайс — только `GET /lk/plans` ✅ real, **без хардкода**; канон цифр —
`pricing.html`, сервер приводит каталог к нему пакетом **S2**. Формы контактов — 🟡 mock (**S3**).
