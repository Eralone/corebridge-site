...
| **Скачивание `.epf`** — токен одноразовый, файл отдаёт nginx по `X-Accel-Redirect` | ✅ все четыре конфигурации, сборки настоящие |
| Бэкенд: пакеты S1–S17 реализованы и на проде | ✅ |
...
2. ✅ **Файлы `.epf` выложены** (2026-07-31). Все четыре конфигурации отдают
   настоящие сборки `v1.0.0`, размеры и контрольные суммы сошлись с таблицей
   сервера, заглушки `0.0.1` сняты с активации. Чинилось в три захода:
   том был примонтирован `:ro` (S16), затем активация звала функцию с чужой
   сигнатурой (S17) — обе правки приехали деплоем, наложение compose снято.
...
7. 🔴 **Сессия теряется при заходе со стороны, письма не уходят вообще** —
   найдено 2026-07-31, промт `prompts/backend_S18_auth_cookie_and_mail.md`.

   Cookie сессии стоит с `SameSite=Strict` и потому не отправляется в переходе
   с чужого сайта — из письма, из мессенджера, с `oauth.yandex.ru`. Сама cookie
   жива; guard видит запрос без неё и уводит на форму входа, а обновление
   страницы не помогает: обновляется уже форма входа. Отсюда и «аккаунт
   не создаётся», и «вход держится только в одном окне».

   | При одной и той же живой сессии | было | стало |
   |---|---|---|
   | адресная строка или закладка | кабинет | кабинет |
   | ссылка с чужого сайта прямо в кабинет | форма входа | кабинет |
   | обновить страницу после этого | форма входа | кабинет |

   На сайте лежит временный обход `lib/auth/session-probe.ts`: форма входа
   спрашивает `GET /lk/auth/session` — обычный `fetch` считается своим, и cookie
   в нём уходит. Настоящее лечение — `sameSite: 'lax'` на сервере, после него
   обход удалить. Заодно шапка публичных страниц теперь знает про вход
   и показывает «Личный кабинет»: в эталоне она была только гостевой, и обратно
   в кабинет с лендинга попасть было нельзя.

   Отдельно: `SMTP_HOST=172.21.0.1` не совпадает с сертификатом Postfix,
   успешных отправок за всё время — **0**.
