...
| **Оплаты** — оформление, возврат с Robokassa, история операций, письма биллинга | ✅ собрано, скрыто флагом до живого тестового платежа |
...
7. ✅ **Сессия и почта починены** (2026-07-31, пакет S18).

   Cookie сессии стояла с `SameSite=Strict` и потому не отправлялась в переходе
   с чужого сайта — из письма, из мессенджера, с `oauth.yandex.ru`. Сама cookie
   была жива; guard видел запрос без неё и уводил на форму входа, а обновление
   не помогало: обновлялась уже форма входа. Отсюда шли сразу две жалобы —
   «аккаунт не создаётся» и «вход держится только в одном окне».
   ...
   Сервер перевёл cookie на `sameSite: 'lax'` и вынес её параметры в общий
   модуль — та же настройка лежала второй копией в приёме приглашений.
   Мой временный обход снят. Из этой истории на сайте осталось полезное:
   шапка публичных страниц теперь знает про вход и показывает «Личный кабинет»
   (в эталоне она только гостевая, и пути обратно в кабинет с лендинга не было).

   Почта не уходила **ни разу за всё время**: `SMTP_HOST=172.21.0.1`
   не совпадал с сертификатом Postfix. Проверено после правки — письма доходят
   до внешнего ящика:

   ```
   регистрация  → email_sent, Postfix: status=sent (250 2.0.0 OK)
   вход по ссылке из письма → открывается сразу в кабинете
   приглашение в команду    → письмо ушло, ссылка открывает форму приёма
   ```

   Флаг `Secure` у cookie сервер отвязал от `NODE_ENV` — теперь он стоит
   по умолчанию, а не зависит от строчки в `.env`.
3. **Оплаты собраны, кнопка спрятана за флагом** (пакет F20). Страницы
   `/billing/pay`, `/billing/success` и `/billing/fail` работают, история платежей
   и категория уведомлений «Оплата и подписка» — по актуальному контракту.
   Этапы сервера P0 и P1 выложены и проверены в контейнерах (`sha-32730bf`,
   миграции `029` и `030`): продление считается от конца оплаченного периода,
   зависшие платежи разбирает сверка через `OpStateExt`, письма об оплате
   и об окончании подписки уходят.

   **Оплата картой открыта 2026-08-06** — `BILLING_PAY_ENABLED=true`
   в `deploy/systemd/corebridge-site.service`. Выключается тем же способом,
   **без пересборки** (флаг читается на сервере при запросе, а не вшивается
   в бандл). Автоплатёж — отдельным флагом `BILLING_AUTOPAY_ENABLED`, он ждёт
   этапа P2 (подписки Robokassa на модерации, планировщика списаний нет).

   ⚠️ **Атрибута «физлицо / компания» у аккаунта нет и не нужно.** Карта и СБП
   доступны всем; заполненный ИНН в настройках — это заявление «плачу
   от компании», по нему дополнительно предлагается счёт по безналу
   (`lib/billing/payer.ts`). Развязка ничего не отбирает: компании платят картой
   так же, как физлица, а тариф и оплаченный срок от реквизитов не зависят.

   **Смена тарифа считается пропорционально** (P3-2, `sha-39357d6`): апгрейд —
   доплата за оставшиеся дни без сдвига даты окончания, даунгрейд — бесплатно
   с продлением срока. Экран оплаты сам решает, какая ветка нужна
   (`POST /lk/billing/change-plan` вместо `/pay`), и показывает расчёт сервера
   до подтверждения. Арифметику не повторяем: между расчётом и оплатой проходит
   время, и второе число разошлось бы с показанным.

   ⚠️ Адреса возврата: в кабинете Robokassa стоят канонические
   `/billing/success` и `/billing/fail`. Прежние `/lk/billing/*` работают
   запасным путём — префикс целиком принадлежит API, поэтому в vhost для них
   заведены точные `location =`, а `middleware.ts` подменяет путь (тем же
   способом, что ссылка из письма-приглашения). У Success/Fail URL метод
   **GET**: межсайтовый POST не несёт cookie `SameSite=Lax`, и клиент после
   оплаты попал бы на форму входа. Переписка с сервером —
   `Documents/server_ask/done/site_F20_billing*.md`.
...
### Э4 — Billing и оплата ← ✅ закрыт пакетом F20 (2026-08-05)

Эндпоинты: `GET /lk/billing`, `POST /lk/billing/pay`, `GET /lk/billing/:public_id` (последний
приезжает с выкладкой P0 — до неё страница возврата берёт статус из истории).

Оплату зовёт **одна страница** — `/billing/pay`. Прайс и биллинг только приводят на неё
с параметрами: у платежа есть выбор периода, промо и разбор семи кодов ошибок, и три копии
этого разошлись бы. Подпись `SignatureValue` считает сервер; на фронте её нет и быть
не может — это проверяет e2e по собранным бандлам, а не обещание в комментарии.

Кнопка «Оплатить» скрыта флагом `BILLING_PAY_ENABLED` до этапа P1 сервера (см. §3 выше),
галочка автоплатежа — флагом `BILLING_AUTOPAY_ENABLED` до P2. Ветка
`{ payment_url: null, message: … }` остаётся и после подключения Robokassa: шлюз может лежать.
...
app/                      Next.js App Router
  ...
  (site)/(lk)/            dashboard, integrations, epf, billing (+ pay/success/fail),
                          settings, support  ← guard сессии
  (admin)/                admin.corebridge.ru ← guard admin-сессии
components/               Sidebar, Topbar, Popup, PublicHeader/Footer, billing/InvoiceRequest
lib/api/                  тонкие fetch-обёртки (auth, lk, admin, public)
lib/billing/              словарь статусов платежа, нормализация ответа, флаги оплаты
...
1. **`/opt/corebridge` не трогаем** — и не читаем как источник истины. Нужны правки бэкенда →
   промт в `Documents/prompts/`. Работающий код сервера смотреть только через
   `docker exec <контейнер>`, схему — через `psql` в `corebridge-postgres`, версию — по тегу
   образа в `docker ps`. Каталоги `services/` и `server/` на VPS не синхронизируются
   с апреля: деплой кладёт туда только `docker-compose.yml`, `scripts/`, `nginx/`, `n8n/`,
   `monitoring/` и `db/`.
