...
1. **Промты S14 и S15 ждут передачи серверу.** S15 блокирующий: вторая фаза
   удаления аккаунта не отрабатывает — `purgeTenant` обезличивает журнал через
   UPDATE, а на `audit_log` стоит триггер неизменяемости. Обращение по 152-ФЗ
   сейчас не довести до конца. S14 — «Повторить» в очереди DLQ всегда падает.
2. **Файлы `.epf` — заглушки.** В `/opt/corebridge/epf/<config>/0.0.1/` лежат
   текстовые файлы: они нужны были, чтобы проверить путь «выбрать конфигурацию →
   получить токен → скачать» целиком. Путь работает, файлы менять на боевые.
