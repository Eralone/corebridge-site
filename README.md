...
7. 🔴 **Вход через Яндекс ID возвращает на форму входа, письма не уходят
   вообще** — найдено 2026-07-31, промт `prompts/backend_S18_auth_cookie_and_mail.md`.
   Cookie сессии стоит с `SameSite=Strict`, поэтому не доезжает при возврате
   с `oauth.yandex.ru`: аккаунт создаётся, а человек видит форму входа.
   На сайте лежит временный обход (`lib/auth/oauth-return.ts`), настоящее
   лечение — `sameSite: 'lax'` на сервере. Отдельно: `SMTP_HOST=172.21.0.1`
   не совпадает с сертификатом Postfix, успешных отправок за всё время — **0**.
