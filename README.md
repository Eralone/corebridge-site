...
| Vitest (29) + Playwright на Firefox (64 всего) | ✅ |
| Инструменты самопроверки: обход сайта + сверка с макетом | ✅ `tools/` |
| Шрифты выложены у себя (CSP резала Google Fonts) | ✅ |
| `sitemap.xml` + `robots.txt`, админ-субдомен закрыт от обхода | ✅ |
| **Э1 Auth: вход, регистрация, восстановление, подтверждение, приглашение** | ✅ расхождение с макетом 0,5–5 % |
| Ссылка из письма-приглашения (`/lk/invite/accept`) | ✅ был 404, починено в vhost + middleware |
| Перенос вёрстки 1:1 (Э2–Э8) | ⬜ |
