...
**Приоритет:** 🔴 без этого выложить `.epf` нельзя: запись падает.
...
**Пересоздавать bridge сейчас бессмысленно** — он поднимется с тем же `:ro`.
Нужен деплой `corebridge-server` с уже сделанной правкой. `/opt/corebridge`
я не правлю руками: автодеплой сотрёт, это ограничение проекта.
